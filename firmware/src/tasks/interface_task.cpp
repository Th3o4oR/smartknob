...
InterfaceTask::InterfaceTask(const uint8_t task_core, const uint32_t stack_depth, MotorTask &motor_task, DisplayTask *display_task, ConnectivityTask &connectivity_task)
    ...
    page_map_[PageID::LIGHTS_RED]    = std::make_unique<LightsPage>(page_context_, connectivity_task_, 0,   "red",    "Red");
    page_map_[PageID::LIGHTS_YELLOW] = std::make_unique<LightsPage>(page_context_, connectivity_task_, 42,  "yellow", "Yellow");
    page_map_[PageID::LIGHTS_GREEN]  = std::make_unique<LightsPage>(page_context_, connectivity_task_, 85,  "green",  "Green");
    page_map_[PageID::LIGHTS_AQUA]   = std::make_unique<LightsPage>(page_context_, connectivity_task_, 128, "aqua",   "Aqua");
    page_map_[PageID::LIGHTS_BLUE]   = std::make_unique<LightsPage>(page_context_, connectivity_task_, 171, "blue",   "Blue");
    page_map_[PageID::LIGHTS_PURPLE] = std::make_unique<LightsPage>(page_context_, connectivity_task_, 213, "purple", "Purple");
